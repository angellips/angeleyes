...
class Script(scripts.Script):
    ...
    def ui(self, is_img2img):
        ...
        output_prompts = gr.Checkbox(label="Output all prompts to a txt file?", value=False) # quick/dirty/buggy

        single_seed = gr.Checkbox(label='Use single seed on all variations', value=False)

        info = gr.HTML(html)
        return [info, is_combinatorial, output_prompts, single_seed]

    def run(self, p, info, is_combinatorial, output_prompts, single_seed):
        fix_seed(p)
        ...
        if single_seed:
            all_seeds = [p.seed for _ in all_prompts]
        else:
        ...
        if output_prompts:
            prompt_txt = open('promptlist.txt', 'w')
            for x in p.prompt:
                prompt_txt.write(x + "\n")
